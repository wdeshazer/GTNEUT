...
C
C  This routine is normally preceded by a call to UMD2IN, to
C  initialize the default control parameters.  UMD2IN need only be
...
C  "INSTALLATION NOTE:" comments in UMD2IN.
...
C               user.  See UMD2IN for details, which sets the defaults.
...
C           Must be set by caller on input (not modified).
C           real control arguments, see UMD2IN for a description,
C           which sets the defaults. UMD2FA uses Cntl (1) and Cntl (2).

C           ------------------------------------------------------------
C  Icntl:   An integer array of size 20.
C           Must be set by caller on input (not modified).
C           Integer control arguments, see UMD2IN for a description,
...
C          n is too small
        ...
        IF (N .GT. NMAX) THEN 
C          n is too big
           CALL UMD2ER (1, ICNTL, INFO, -1, NMAX) 
           GO TO 9000
        ENDIF 
        IF (NE .LT. 1) THEN 
C          ne is too small
